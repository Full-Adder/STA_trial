...
def get_dataLoader(Pic_path, H5_path, GT_path, train_mode, STA_mode,
                   batch_size, input_size, crop_size, ST_cut=0):
    ...
    if train_mode == "train":
        img_train = AVEDataset(pic_dir=Pic_path, h5_dir=H5_path, gt_dir=GT_path, STA_mode=STA_mode,
                               mode="train", ST_cut=ST_cut, transform=tsfm_train, transforms_gt=tsfm_gt)
        ...
        img_test = AVEDataset(pic_dir=Pic_path, h5_dir=H5_path, gt_dir=GT_path,STA_mode=STA_mode,
                              mode="test", ST_cut=ST_cut, transform=tsfm_test, transforms_gt=tsfm_gt)
        ...
        img_val = AVEDataset(pic_dir=Pic_path, h5_dir=H5_path, gt_dir=GT_path, STA_mode=STA_mode,
                             mode="val", ST_cut=ST_cut, transform=tsfm_test, transforms_gt=tsfm_gt)
        ...
        img_all = AVEDataset(pic_dir=Pic_path, h5_dir=H5_path, gt_dir=GT_path, STA_mode=STA_mode,
                             mode="all", ST_cut=ST_cut, transform=tsfm_test, transforms_gt=tsfm_gt)
