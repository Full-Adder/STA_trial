...
def test(model, Pic_path, is_val, save_index, batch_size, input_size, dataset_name, SummaryWriter_dir=r"./log",
         test_dir=None):
    ...
    if is_val:
        save_path_hh = './test/'+str(save_index)
    else:
        save_path_hh = test_dir
    ...
    for idx_test, dat_test in enumerate(test_loader):
        ...
        if is_val:
            writer.add_scalars(dataset_name, {"val_loss": loss_t.data.item()},
                               save_index * len(test_loader) * 8 + idx_test)
        ...
        for i in range(ind.shape[0]):  # 非0元素的个数
            ...
            if is_val or la == index_of_pic[i]:  # 如果标签中非0索引==最大结果的索引
                ...
                if is_val and idx_test % 100 == 66 and i == 5:
                    writer.add_images(os.path.split(img_name1)[-1], att, save_index)
...
if __name__ == '__main__':
    ...
    test(model=net, Pic_path=args.Pic_path, is_val=False, save_index=0, batch_size=args.batch_size,
         input_size=args.input_size, dataset_name=args.dataset_name, SummaryWriter_dir=args.SummaryWriter_dir,
         test_dir=r'./test_result')
