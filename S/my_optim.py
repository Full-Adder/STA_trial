...
class PolyOptimizer(optim.SGD):
    ...
    def __init__(self, params, lr, weight_decay, max_step, momentum=0.9):
        super().__init__(params, lr, weight_decay) # 继承父类，初始化参数
        self.param_groups = params
        self.global_step = 0        # 全局次数
        self.max_step = max_step    # 最大迭代次数
        self.momentum = momentum    # 动量
        self.__initial_lr = [group['lr'] for group in self.param_groups]  # 最初的lr

    def step(self, closure=None):       # 优化一次
...
def lr_poly(base_lr, iter, max_iter, power=0.9):    # lr多项式
...
def reduce_lr(args, optimizer, epoch, factor=0.1):
    values = args.decay_points.strip().split(',')  # 衰减点，来自命令行输入参数 defult = [5,10]
    try:
        change_points = map(lambda x: int(x.strip()), values)  # 取出value
    except ValueError:
        change_points = None

    if change_points is not None and epoch in change_points:  # 如果当前点属于衰减点
        for g in optimizer.param_groups:
            g['lr'] = g['lr'] * factor      # 所有lr减少10倍
            print(epoch, g['lr'])
        return True
